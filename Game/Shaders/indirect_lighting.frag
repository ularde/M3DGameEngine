...
void main()
{
    ...
    for(int i = 0; i < 32; ++i)
    {
        ...
        vec3 transmittance = normalize(fragPos - samplePos);
        float cosThetaSi = max(dot(transmittance, sampleNormal), 0.0);
        float cosThetaRi = max(dot(-transmittance, normal), 0.0);
        Ldi += ((sampleDepth >= samplePos.z + bias) && (rangeCheck >= 1.0) ? 1.0 : 0.0) * calculateIndirectLight(cosThetaSi, cosThetaRi, di, sampleColor); 
    }
    occlusion = 1.0 - (occlusion / 32);
    fragColor = vec4(Ldi * 2.0, occlusion);
}