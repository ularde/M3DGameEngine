...
static CString rootDirFullPath;
...
int CFileView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	HTREEITEM hRoot = m_wndFileView.InsertItem(_T("Assets"), 0, 0);
	m_wndFileView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	CFileFind rootDirGetter;
	rootDirGetter.FindFile(_T(".\\..\\..\\Game\\*.*"));
	rootDirGetter.FindNextFile();
	rootDirFullPath = rootDirGetter.GetRoot();

	FillFileView(hRoot, _T(".\\..\\..\\Game\\"));
...
void CFileView::FillFileView(HTREEITEM ftree, CString fn)
{
	static HTREEITEM hFolder = 0;
	CFileFind finder;
	if (ftree != 0)
	{
		...
		while (working)
		{
			...
			{
				if (!(finder.IsDots() || finder.IsHidden() || finder.IsReadOnly() || finder.IsSystem() || finder.IsTemporary()))
				{
					hFolder = m_wndFileView.InsertItem(finder.GetFileName(), 0, 0, ftree);
					FillFileView(hFolder, fpath + _T("\\"));
				}
			}
		}
	}
...
void CFileView::OnFileOpen()
{
	...
	else if (ename == _T(".ment"))
	{

	}
	else if (ename == _T(".mmtl"))
	{
...
void CFileView::FindFiles(std::vector<CString>& fpaths, CString dir)
{
	CFileFind finder;
	BOOL isNotEmpty = finder.FindFile(dir + _T("*."));
	while (isNotEmpty)
	{
